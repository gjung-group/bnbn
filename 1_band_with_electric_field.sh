...
tt=`awk '$0 !~/^$/{a=$0;}END{if($0 ~/^$/) print a;else print $1;}' BLBL.mol`
per=`echo "scale=0;$tt/4" | bc -l`
dd=`awk '{print $1}' dis.txt`
ele=`echo "scale=8;0.82275/2*$dd*$i*0.05" |bc -l`
#E_tb=`echo "scale=8;(1.753584+$ele)*$per" |bc -l`
#E_tn=`echo "scale=8;(-2.293102+$ele)*$per" |bc -l`
#E_bb=`echo "scale=8;(1.753584-$ele)*$per" |bc -l`
#E_bn=`echo "scale=8;(-2.293102-$ele)*$per" |bc -l`
#echo $E_tb
cat > bndpele.py << EOF
...
onsite = [1.753584+$ele]*$per+[-2.293102+$ele]*$per+[1.753484-$ele]*$per+[-2.293102-$ele]*$per
onsite = onsite
